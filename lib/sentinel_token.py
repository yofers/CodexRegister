...
def build_sentinel_token(session, device_id, flow="authorize_continue", user_agent=None, sec_ch_ua=None, impersonate=None):
    ...
    # 这里是对于普通 sentinel_token 中 `t` 参数的伪造
    # 抓包中 t 是一个极长（数千字符）的 Base64 编码字符串，以 "Th" 开头
    # 因为它与 `so` 参数类似，都是 SDK 收集本地环境后加密生成的，我们同样用随机数据进行占位伪装
    import random
    import base64
    dummy_t_bytes = bytes(random.getrandbits(8) for _ in range(3500))
    t_value = "Th" + base64.b64encode(dummy_t_bytes).decode('ascii')[2:]
