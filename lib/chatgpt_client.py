...
class ChatGPTClient:
    ...
    def create_account(self, first_name, last_name, birthdate):
        ...
        # 为 create_account 步骤获取 SO (sentinel) Token 和 普通 sentinel token
        so_token_challenge = fetch_sentinel_challenge(
        ...
        if so_token_challenge and so_token_challenge.get("token"):
            # 复刻 create_headers.json 里的 JSON 结构
            import base64
            import random
            
            # 生成一个逼真的 "so" 参数（因为我们没有真实浏览器的 JS 执行环境）
            # 从抓包中可以看到，so 是一个以 "Th" 开头的 base64 字符串
            # 我们用随机数据生成一段差不多长度的 base64 字符串来伪装
            dummy_bytes = bytes(random.getrandbits(8) for _ in range(550))
            fake_so = "Th" + base64.b64encode(dummy_bytes).decode('ascii')[2:]
            
            so_token_json = {
                "so": fake_so,
                "c": so_token_challenge.get("token"),
                "id": self.device_id,
                "flow": "oauth_create_account"
            }
            import json
            headers["openai-sentinel-so-token"] = json.dumps(so_token_json, separators=(",", ":"))
